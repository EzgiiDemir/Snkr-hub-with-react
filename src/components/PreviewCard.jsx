...
const PreviewCard = ({ shoe }) => {
  ...
  const fav = () => {
    const shoeInCart = cart.some((item) => item.id === shoe.id);
    if (shoeInCart) {
      toast.error("You've Already Added Favourites This Item");
    } else {
      dispatch(addToCart(shoe));
      toast.success("Added to favourites cart");
    }
  };
      ...
      <main className="grid place-items-center min-h-[1300px] md:min-h-screen bg-gray-50 dark:bg-[#121212]">
        <section className="flex flex-col md:flex-row gap-11 py-10 px-5 bg-white dark:bg-[#1f1b24] dark:hover:bg-[#121015]  rounded-xl shadow-xl hover:shadow-2xl  w-3/4 md:max-w-2xl">
          ...
          <main className="text-gray-500 dark:text-white">
            ...
            <div class="flex gap-x-6"><p class="flex "><button class=" mr-1 bg-[#dadada] dark:bg-[#2a2a2a] dark:hover:bg-black dark:border-none border rounded-lg font-bold w-[30px]">-</button><span class="text-lg font-bold">1</span><button class=" ml-1 bg-[#dadada] dark:bg-[#2a2a2a] dark:hover:bg-black dark:border-none border rounded-lg font-bold w-[30px]">+</button></p></div>
            <div className="flex gap-0.5 mt-4">
              <button
                id="addToCartButton"
                className="bg-[#2a2a2a] hover:bg-black focus:outline-none transition text-white uppercase px-3 py-1"
                onClick={add}
              >
                add to cart
              </button>
              <button
                id="likeButton"
                className="bg-[#2a2a2a] hover:bg-black focus:outline-none transition text-white uppercase p-1"
                onClick={fav}
              >
                <svg
              ...
              <button
                className="bg-[#2a2a2a] hover:bg-black focus:outline-none transition text-white uppercase px-3 py-1"
                onClick={() => navigate(-1)} // -1 değeri ile bir önceki sayfaya gitmek için
              >
                Back
              </button>
            </div>
